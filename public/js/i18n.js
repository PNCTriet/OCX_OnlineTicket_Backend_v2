...
class I18n {
    constructor() {
        ...
        console.log('[i18n] Checking DOM state');
        // Initialize when DOM is ready
        if (document.readyState === 'loading') {
            console.log('[i18n] DOM loading, waiting for DOMContentLoaded');
            document.addEventListener('DOMContentLoaded', () => this.init());
        } else {
            console.log('[i18n] DOM ready, initializing immediately');
            this.init();
        }
    }
    ...
    init() {
        ...
        try {
            ...
            console.log('[i18n] Initialization complete, dispatching i18nReady event');
            // Dispatch event that i18n is ready
            document.dispatchEvent(new Event('i18nReady'));
        } catch (error) {
            console.error('[i18n] Initialization error:', error);
            // Even if there's an error with display/listeners, we still want i18n to be usable
            this.isInitialized = true;
            document.dispatchEvent(new Event('i18nReady'));
        }
    }
