...
class DashboardModule {
    constructor() {
        ...
        console.log('[Dashboard] Token exists:', !!this.token);
        console.log('[Dashboard] User exists:', !!this.currentUser);
        ...
        if (document.readyState === 'loading') {
            ...
            console.log('[Dashboard] DOM already loaded, proceeding to waitForDependencies');
            this.waitForDependencies();
        }
    }

    waitForDependencies() {
        console.log('[Dashboard] waitForDependencies called');
        console.log('[Dashboard] i18n.isInitialized:', i18n.isInitialized);
        
        // Add timeout to prevent infinite loading
        const timeout = setTimeout(() => {
            console.error('[Dashboard] Initialization timeout - forcing page reload');
            window.location.reload();
        }, 10000); // 10 second timeout

        // Check if i18n is ready
        if (i18n.isInitialized) {
            console.log('[Dashboard] i18n already initialized, proceeding to init');
            clearTimeout(timeout);
            this.init();
        } else {
            console.log('[Dashboard] Waiting for i18n to be ready...');
            document.addEventListener('i18nReady', () => {
                console.log('[Dashboard] i18nReady event received');
                clearTimeout(timeout);
                if (!this.isInitialized) {
                    console.log('[Dashboard] Proceeding to init');
                    this.init();
                } else {
    ...
    async init() {
        ...
        if (this.isInitialized) {
            console.log('[Dashboard] Already initialized, returning');
            return;
        }
        this.isInitialized = true;
        console.log('[Dashboard] Set isInitialized to true');
        ...
        try {
            ...
            if (!this.checkAdminAccess()) {
                console.log('[Dashboard] Not admin access, showing alert and redirecting');
            ...
            console.log('[Dashboard] Hiding loading screen');
            this.hideLoading();
        }
    }
    ...
    async loadComponents() {
        try {
            console.log('[Dashboard] Loading sidebar component');
            await componentLoader.loadComponent('sidebar', 'sidebar-container');
            console.log('[Dashboard] Loading header component');
            await componentLoader.loadComponent('header', 'header-container');
            console.log('[Dashboard] Loading alert component');
            await componentLoader.loadComponent('alert', 'alert-container');
            console.log('[Dashboard] All components loaded successfully');
        } catch (error) {
            console.error('[Dashboard] Error loading components:', error);
            throw error;
        }
    }
